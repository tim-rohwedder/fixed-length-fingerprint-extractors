...
from src.generate_embeddings_alignment import get_alignment_dataset_name
from src.visualization.plot_heatmap import plot_heatmap
from src.setup.paths import get_figures_dir

import random

def _get_experiment_with_rw(testset_key: str, extractor_key: str, rotation:  int, shift: int) -> list[Experiment]:
    _, _, experiments = get_experiments(
        testset_keys=[testset_key],
        extractor_keys=[extractor_key]
    )
    ...
    rw_experiment.dataset_name = get_alignment_dataset_name(testset_key, rotation, shift)
...
def main():
    ...
    EXTRACTOR_KEY = "DeepFinger_TexMinu_512"
    ...
    rw_results_mat: np.array = np.zeros((len(ROTATION_MAGNITUDES), len(SHIFT_MAGNITUDES)))
    for i, r in enumerate(ROTATION_MAGNITUDES):
        for j, s in enumerate(SHIFT_MAGNITUDES):
            experiment, rw_experiment = _get_experiment_with_rw(DATASET_NAME, EXTRACTOR_KEY, r, s)
    ...
    for mat, filename in [(results_mat, f"{EXTRACTOR_KEY}.png"), (rw_results_mat, f"{EXTRACTOR_KEY}_rw.png")]:
        plot_heatmap(
            mat, 
