...
from src.benchmarks.verification import VerificationResult
from src.benchmarks.identification import IdentificationResult
from src.setup.experiments import get_experiments, get_reweighting_experiments, Experiment, DatasetLoader
from src.setup.paths import get_figures_dir
from src.visualization.plot_DET_curve import (
    plot_verification_results,
    plot_identification_results,
)
from src.visualization.plot_ranks import plot_rank_n_identification_rates
from src.visualization.plot_distribution_scores import plot_similarity_scores_results
...
def _plot_results_on_dataset(
    ...
    if config.plot_distributions and not SKIP_DISTRIBUTIONS:
        ...
        plot_similarity_scores_results(
            ...
            ] if MAKE_TITLE else [""] * len(model_labels),
        )
    ...
    if config.plot_verification and not SKIP_VERIFICATION:
        ...
        plot_verification_results(
            ...
            plot_title=f"Verification performance on {dataset.label}" if MAKE_TITLE else "",
        )
    ...
    if config.plot_closed_set and not SKIP_CLOSED_SET:
        ...
        plot_rank_n_identification_rates(
            ...
            results=results,
            model_labels=model_labels,
            plot_title=f"Rank-N identification rates on {dataset.label}" if MAKE_TITLE else "",
        )
    ...
    if config.plot_open_set and not SKIP_OPEN_SET:
        ...
        plot_identification_results(
            ...
            results=results,
            model_labels=model_labels,
            plot_title=f"Identification performance on {dataset.label}" if MAKE_TITLE else "",
        )


def _get_experiments_with_rw(testset_keys: list[str], extractor_keys: list[str], embedding_types: Union[EmbeddingType, list[EmbeddingType]]):
    ...
    orig_experiments = {k:e for k, e in experiments.items() if embedding_types[k[0]] == EmbeddingType.Original or embedding_types[k[0]] == EmbeddingType.Both}
    rw_experiments = get_reweighting_experiments({k:e for k, e in experiments.items() if embedding_types[k[0]] == EmbeddingType.Reweighted or embedding_types[k[0]] == EmbeddingType.Both})
...
def main():
    for plot_config in PLOT_CONFIGS:
        datasets, experiments_lst = _get_experiments_with_rw(TESTSET_KEYS, plot_config.extractor_keys, plot_config.embedding_types)
