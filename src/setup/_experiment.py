...
from src.benchmarks.identification import IdentificationBenchmark, IdentificationResult
from src.data.embedding_dataset import EmbeddingDataset, combine_embeddings_if_both_exist
...
class Experiment:
    ...
    @staticmethod
    def _load_embeddings_if_exist(path: str) -> Union[None, EmbeddingDataset]:
        if not os.path.exists(path) or len(os.listdir(path)) == 0:
            return None
        return EmbeddingDataset.load(path)

    def load_training_embeddings(self) -> EmbeddingDataset:
        assert self.reweighting_training_indices is not None
        all_embeddings = self.load_embeddings()
        train_ids = (all_embeddings.ids[idx] for idx in self.reweighting_training_indices)
        return EmbeddingDataset([all_embeddings.get(bid) for bid in train_ids])

    def load_embeddings(self) -> EmbeddingDataset:
        ...
        return combine_embeddings_if_both_exist(tex_embeddings, minu_embeddings)

    def save_embeddings(
        self,
        texture_embeddings: EmbeddingDataset,
        minutia_embeddings: EmbeddingDataset,
    ):
        embeddings_base_dir = get_generated_embeddings_dir(
            self.model_name, self.dataset_name
        )
        if texture_embeddings is not None:
            texture_embeddings.save(get_texture_embedding_dataset_dir(embeddings_base_dir))
        if minutia_embeddings is not None:
            minutia_embeddings.save(get_minutia_embedding_dataset_dir(embeddings_base_dir))
...
class ReweightingExperiment(Experiment):
    ...
    def __init__(self, model_name: str, model_label: str, **kwargs):
        super().__init__(
            ...
            model_label=model_label + self.REWEIGHTED_EXTRACTOR_LABEL_POSTFIX, **kwargs)

    def load_embeddings(self) -> EmbeddingDataset:
        embeddings_base_dir = get_generated_embeddings_dir(
            self.model_name[:-len(self.REWEIGHTED_EXTRACTOR_NAME_POSTFIX)], self.dataset_name
        )
        return EmbeddingDataset.load(
            get_reweighted_embedding_dataset_dir(embeddings_base_dir)
        )

    def save_embeddings(
        self,
        reweighted_embeddings: EmbeddingDataset,
    ):
        embeddings_base_dir = get_generated_embeddings_dir(
            self.model_name[:-len(self.REWEIGHTED_EXTRACTOR_NAME_POSTFIX)], self.dataset_name
        )
        reweighted_embeddings.save(get_reweighted_embedding_dataset_dir(embeddings_base_dir))
