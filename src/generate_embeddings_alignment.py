...
from src.preprocessing.augmentation import RandomPoseTransform
from src.setup.experiments import get_experiments, DatasetLoader, FixedLengthExtractorLoader, Experiment
from copy import deepcopy
from src.reweighting.reweight_embeddings import reweight_embeddings

def _get_rotation_transforms(angle_deviation: int, shift_deviation: int) -> list[RandomPoseTransform]:
...
def get_alignment_dataset_name(dataset_name: str, angle_deviation: int, shift_deviation: int) -> str:
...
def main():
    ...
    testsets, extractors, experiments = get_experiments(
        ...
        "mcyt330_optical",
        #"mcyt330_capacitive",
    ], extractor_keys=["DeepFinger_TexMinu_512"]
    )
        ...
        for extractor in extractors:
            for angle_deviation in ROTATION_MAGNITUDES:
                for shift_deviation in SHIFT_MAGNITUDES:
                    ...
                    dataloader_mod.kwargs = {"poses": _get_rotation_transforms(angle_deviation, shift_deviation), "only_last_n": 1300}

                    # Create new experiment with modified dataset name
                    experiment_mod: Experiment = experiments[(extractor.name, dataloader.name)]
                    experiment_mod.dataset_name = get_alignment_dataset_name(dataloader.name, angle_deviation, shift_deviation)
                    new_experiments[(extractor.name, experiment_mod.dataset_name)] = experiment_mod
