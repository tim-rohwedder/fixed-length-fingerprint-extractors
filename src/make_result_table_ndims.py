...
def _get_experiments_with_rw(testset_keys: list[str], extractor_keys: list[str]) -> list[Experiment]:
...
def _make_row(dims: int, ops: int, fnmr: float, fnmr_rw: float, eer: float, eer_rw: float) -> str:
    outstr = f"& {dims} & {ops} & "
...
def main():
    ...
    for e in EXTRACTOR_KEYS:
        ...
        capacitive, capacitive_rw = _get_experiments_with_rw(["mcyt330_capacitive"], [e])
