from src.models import InceptionV4
import torch
from torch import nn

from src.models.localization_network import LocalizationNetwork
"""
Terminology:
...
class _InceptionV4_Stem(nn.Module):
    ...
    def forward(self, input):
        assert input.shape[-1] == DEEPFINGER_INPUT_SIZE
        assert input.shape[-2] == DEEPFINGER_INPUT_SIZE
...
class DeepFingerTrainingOutput(DeepFingerOutput):
    def __init__(
        self,
...
class DeepFinger_Tex(nn.Module):
    """
    ...
    def forward(self, input) -> torch.Tensor:
        if self.training:
            ...
            return DeepFingerTrainingOutput(texture_logits=logits, texture_embeddings=x)
...
class DeepFinger_Minu(nn.Module):
    """
    ...
    def forward(self, input: torch.Tensor) -> DeepFingerOutput:
        if self.training:
            x = self.stem(input)
        ...
        with torch.no_grad():
            ...
            x_minutia_emb = self.minutia_embedding.forward(x_minutia)
            return DeepFingerOutput(minutia_embeddings=x_minutia_emb)


class DeepFinger_TexMinu(nn.Module):
    """
    ...
    def forward(self, input: torch.Tensor) -> DeepFingerOutput:
        ...
        with torch.no_grad():
            ...
            x_minutia_emb = self.minutia_embedding.forward(x_minutia)
            return DeepFingerOutput(x_minutia_emb, x_texture_emb)


class DeepFinger_TexMinuCombi(nn.Module):
    """
    Model with texture and minutia branch. The layer before logits of both branches are concatenated and
    one logit loss for both branches combined is calculated.
    ...
    def __init__(
        ...
        self.logits = nn.Sequential(
            nn.Linear(texture_embedding_dims + minutia_embedding_dims, num_fingerprints), nn.Dropout(p=0.2)
        )
    ...
    def forward(self, input: torch.Tensor) -> DeepFingerOutput:
        if self.training:
            ...
            x_logits = self.logits(torch.concatenate((x_texture_emb, x_minutia_emb), dim=1))

            return DeepFingerTrainingOutput(
                minutia_embeddings=x_minutia_emb,
                texture_embeddings=x_texture_emb,
                combined_logits=x_logits,
                minutia_maps=x_minutia_map,
            )
...
class DeepFinger_LocTex(nn.Module):
    """
    ...
    def __init__(self, num_fingerprints, texture_embedding_dims: int):
        ...
        self.embeddings = DeepFinger_Tex(
...
class DeepFinger_LocMinu(nn.Module):
    """
    ...
    def __init__(self, num_fingerprints, minutia_embedding_dims: int):
        ...
        # Modules
        self.localization = LocalizationNetwork()
        self.embeddings = DeepFinger_Minu(
            num_fingerprints=num_fingerprints,
...
class DeepFinger_LocTexMinu(nn.Module):
    """
    ...
    def __init__(
        ...
        # Modules
        self.localization = LocalizationNetwork()
        self.embeddings = DeepFinger_TexMinu(
            num_fingerprints=num_fingerprints,
            texture_embedding_dims=texture_embedding_dims,
            minutia_embedding_dims=minutia_embedding_dims,
        )
...
def main():
    model = DeepFinger_Tex(1000, 100)
    model = DeepFinger_Minu(1000, 100)
    model = DeepFinger_TexMinu(1000, 100, 100)
    model = DeepFinger_LocTex(1000, 100)
    model = DeepFinger_LocMinu(1000, 100)
    model = DeepFinger_LocTexMinu(1000, 100, 100)
